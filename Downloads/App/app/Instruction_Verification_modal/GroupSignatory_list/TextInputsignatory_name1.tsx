...
const TextInputsignatory_name1 = ({checkToAdd,setCheckToAdd,refetch,setRefetch}:any) => {
  ...
  const {globalState , setGlobalState} = useContext(TotalContext) as TotalContextProps
    ...
    const handleChange = (e: any) => {
             setSignatory_list7d3b5((prev: any) => ({ ...prev, signatory_name1: e.target.value }))


    }
    if(Signatory_list7d3b5?.signatory_name1){
      let update=(e:any)=>{
               setSignatory_list7d3b5((prev: any) => ({ ...prev, signatory_name1: e.target.value }))
      }
    }
...
*/
     ...
     let getMapperDetails:any =  await AxiosService.post("/UF/getMapperDetails",getMapperDetailsBody,{
      ...
      if(Array.isArray(getMapperDetails?.data))
      {
        ...
        if(filteredData.length){
          ...
          setSignatory_list7d3b5((pre:any)=>({...pre,signatory_name1:filteredData[0]?.signatory_name1}))
        }
        return
      }else{
        setSignatory_list7d3b5((pre:any)=>({...pre,signatory_name1:getMapperDetails?.data}))
      }
    }
    catch(err)
    {
  ...
  return (   
    <div className="col-start-5 col-end-10 row-start-8 row-end-9 gap-" >
      <TextInput
        label={keyset("")}
        onChange= {handleChange}
        onBlur={handleBlur}
        type="text"
        value={Signatory_list7d3b5?.signatory_name1||""}
...
export default TextInputsignatory_name1