...
const Imagesignatory_signature_url1 =  () => {
  ...
  const handleMapperValue=async()=>{
        let getMapperDetailsBody:any={
          ...
          controlName:"signatory_signature_url1",
        }
        try{
         let getMapperDetails:any =  await AxiosService.post("/UF/getMapperDetails",getMapperDetailsBody,{
          ...
          if(Array.isArray(getMapperDetails?.data))
          {
            ...
            if(filteredData.length){
              ...
              setSignatory_list7d3b5((pre:any)=>({...pre,signatory_signature_url1:filteredData[0]?.signatory_signature_url1}))
            }
            else {
              
              setSignatory_list7d3b5((pre:any)=>({...pre,signatory_signature_url1:""}))
            }
            return
          }else{
            setSignatory_list7d3b5((pre:any)=>({...pre,signatory_signature_url1:getMapperDetails?.data}))
          }
        }
        catch(err)
        {
  ...
  return (
    <div className="col-start-5 col-end-10 row-start-6 row-end-7 gap-" >
      <div>{Signatory_list7d3b5?.signatory_signature_url1==""||Signatory_list7d3b5?.signatory_signature_url1==undefined?
        <img
          src={Signatory_list7d3b5?.signatory_signature_url1||""}
        alt={" no image found"}
      />
      :
      <img
          src={Signatory_list7d3b5?.signatory_signature_url1||""}
        alt={"signatory_signature_url1"}
      />
        }
...
export default Imagesignatory_signature_url1
