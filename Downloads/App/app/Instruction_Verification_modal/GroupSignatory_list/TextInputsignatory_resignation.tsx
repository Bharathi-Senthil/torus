...
const TextInputsignatory_resignation = ({checkToAdd,setCheckToAdd,refetch,setRefetch}:any) => {
  ...
  const {globalState , setGlobalState} = useContext(TotalContext) as TotalContextProps
    ...
    const handleChange = (e: any) => {
             setSignatory_list7d3b5((prev: any) => ({ ...prev, signatory_resignation: e.target.value }))


    }
    if(Signatory_list7d3b5?.signatory_resignation){
      let update=(e:any)=>{
               setSignatory_list7d3b5((prev: any) => ({ ...prev, signatory_resignation: e.target.value }))
      }
    }
...
*/
  ...
  const handleMapperValue=async()=>{
   let getMapperDetailsBody:any={
      ...
      controlName:"signatory_resignation",
    }
    try{
     let getMapperDetails:any =  await AxiosService.post("/UF/getMapperDetails",getMapperDetailsBody,{
      ...
      if(Array.isArray(getMapperDetails?.data))
      {
        ...
        if(filteredData.length){
          ...
          setSignatory_list7d3b5((pre:any)=>({...pre,signatory_resignation:filteredData[0]?.signatory_resignation}))
        }
        return
      }else{
        setSignatory_list7d3b5((pre:any)=>({...pre,signatory_resignation:getMapperDetails?.data}))
      }
    }
    catch(err)
    {
  ...
  return (   
    <div className="col-start-5 col-end-10 row-start-4 row-end-5 gap-" >
      <TextInput
        label={keyset("")}
        onChange= {handleChange}
        onBlur={handleBlur}
        type="text"
        value={Signatory_list7d3b5?.signatory_resignation||""}
...
export default TextInputsignatory_resignation