...
const TextInputinstruction_detail = ({checkToAdd,setCheckToAdd,refetch,setRefetch}:any) => {
  ...
  const {globalState , setGlobalState} = useContext(TotalContext) as TotalContextProps
    ...
    const handleChange = (e: any) => {
             setInstruction_verification10e47((prev: any) => ({ ...prev, instruction_detail: e.target.value }))


    }
    if(Instruction_verification10e47?.instruction_detail){
      let update=(e:any)=>{
               setInstruction_verification10e47((prev: any) => ({ ...prev, instruction_detail: e.target.value }))
      }
    }
...
*/
  ...
  const handleMapperValue=async()=>{
   let getMapperDetailsBody:any={
      ...
      controlName:"instruction_detail",
    }
    try{
     let getMapperDetails:any =  await AxiosService.post("/UF/getMapperDetails",getMapperDetailsBody,{
      ...
      if(Array.isArray(getMapperDetails?.data))
      {
        ...
        if(filteredData.length){
          ...
          setInstruction_verification10e47((pre:any)=>({...pre,instruction_detail:filteredData[0]?.instruction_detail}))
        }
        return
      }else{
        setInstruction_verification10e47((pre:any)=>({...pre,instruction_detail:getMapperDetails?.data}))
      }
    }
    catch(err)
    {
  ...
  return (   
    <div className="col-start-1 col-end-4 row-start-7 row-end-8 gap-" >
          <Label theme="clear">{keyset("Instruction Detail")}</Label>
      <TextInput
        onChange= {handleChange}
        onBlur={handleBlur}
        type="text"
        value={Instruction_verification10e47?.instruction_detail||""}
...
export default TextInputinstruction_detail