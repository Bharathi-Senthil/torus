...
const TextInputaccount_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch}:any) => {
  ...
  const {globalState , setGlobalState} = useContext(TotalContext) as TotalContextProps
    ...
    const handleChange = (e: any) => {
             setInstruction_verification10e47((prev: any) => ({ ...prev, account_name: e.target.value }))


    }
    if(Instruction_verification10e47?.account_name){
      let update=(e:any)=>{
               setInstruction_verification10e47((prev: any) => ({ ...prev, account_name: e.target.value }))
      }
    }
...
*/
     ...
     let getMapperDetails:any =  await AxiosService.post("/UF/getMapperDetails",getMapperDetailsBody,{
      ...
      if(Array.isArray(getMapperDetails?.data))
      {
        ...
        if(filteredData.length){
          ...
          setInstruction_verification10e47((pre:any)=>({...pre,account_name:filteredData[0]?.account_name}))
        }
        return
      }else{
        setInstruction_verification10e47((pre:any)=>({...pre,account_name:getMapperDetails?.data}))
      }
    }
    catch(err)
    {
  ...
  return (   
    <div className="col-start-5 col-end-8 row-start-4 row-end-5 gap-" >
          <Label theme="clear">{keyset("Account Name")}</Label>
      <TextInput
        onChange= {handleChange}
        onBlur={handleBlur}
        type="text"
        value={Instruction_verification10e47?.account_name||""}
...
export default TextInputaccount_name