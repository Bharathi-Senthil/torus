...
const TextInputaccount_number = ({checkToAdd,setCheckToAdd,refetch,setRefetch}:any) => {
  ...
  const {globalState , setGlobalState} = useContext(TotalContext) as TotalContextProps
    ...
    const handleChange = (e: any) => {
             setInstruction_verification10e47((prev: any) => ({ ...prev, account_number: e.target.value }))


    }
    if(Instruction_verification10e47?.account_number){
      let update=(e:any)=>{
               setInstruction_verification10e47((prev: any) => ({ ...prev, account_number: e.target.value }))
      }
    }
...
*/
     ...
     let getMapperDetails:any =  await AxiosService.post("/UF/getMapperDetails",getMapperDetailsBody,{
      ...
      if(Array.isArray(getMapperDetails?.data))
      {
        ...
        if(filteredData.length){
          ...
          setInstruction_verification10e47((pre:any)=>({...pre,account_number:filteredData[0]?.account_number}))
        }
        return
      }else{
        setInstruction_verification10e47((pre:any)=>({...pre,account_number:getMapperDetails?.data}))
      }
    }
    catch(err)
    {
  ...
  return (   
    <div className="col-start-1 col-end-4 row-start-4 row-end-5 gap-" >
          <Label theme="clear">{keyset("account_number")}</Label>
      <TextInput
        onChange= {handleChange}
        onBlur={handleBlur}
        type="text"
        value={Instruction_verification10e47?.account_number||""}
...
export default TextInputaccount_number